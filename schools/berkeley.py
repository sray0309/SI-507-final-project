...
def check_data():
    ...
    if (faculty['cache_time'][:7] != time_now[:7] or faculty['total_number'] == 0):
        ...
        delete_old_data = '''
            DELETE FROM faculty
            WHERE faculty.School = "berkeley"
        '''
        cursor.execute(delete_old_data)
        connection.commit()
        for data in faculty['detail']:
            update_data = f'''
            INSERT INTO faculty ("FirstName", "LastName", "School", "Title", "ResearchInterests", "PersonalWeb", "Email") VALUES("{data['firstname']}", "{data['lastname']}", "berkeley", "{data['title']}", "{data['research_interests']}", "{data['personal_web']}", "{data['email']}")
            '''
